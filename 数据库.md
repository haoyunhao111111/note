...
```mysql
grant privileges all on privilege_level(作用于表)
...
| 兼容情况    0=冲突  1=兼容 | X(排它锁) | IX   | S(共享锁) | IS   |
| -------------------------- | --------- | ---- | --------- | ---- |
| X                          | 0         | 0    | 0         | 0    |
| IX                         | 0         | 1    | 0         | 1    |
| S                          | 0         | 0    | 1         | 1    |
...
```mysql
GRANT REPLICATION SLAVE ON *.* to 'root'@'192.168.184.130' identifided by '123321';
flush privileges;
```
...
change master to master_host='192.168.2.179', master_user='root', master_password='123123',master_log_file='mysql-bin.000003', master_log_pos=512; 
...
- 查看系统变量log_bin
- 开启二进制日志，必须在my.cnf中[mysqld]下面添加log_bin [=DIR[filename]],DIR指定二进制文件的存储文件，filename指定二进制文件的文件名
- 系统变量log_bin是静态参数，不能动态修改
- 修改my.cnf，在[mysqld]下面增加log_bin=mysql_bin_log,重启mysql后，你就会发现log_bin变为了ON，二进制日志【binary】默认放在数据目录下，系统变量datadir下
...
```mysql
purge binary logs to 'mysq-bin.000002'
```
...
> 查询语句是使用Mysql的频率最高的语句，也是影响性能最重要的环节
...
- id: sql执行的顺序标识
  - id相同时，执行顺序由上到下
  - 如果是子查询，id的序号会递增id值越大，越先执行

- select_type:查询语句select字句的类型
...
- type:mysql在表中找到所需行的方式
  ...
  > all<index<range<ref<bxxxxconst<system<NULL

  - all:  Full Table Scan,mysql将遍历全表以找到匹配的行并且查到的内容不带索引
  - index:  Full Index scan,index与all的区别为index类型只遍历索引树，也就是查询有索引的树
  - range:只检索给定范围的行，查找的内容不带索引，选择的行带索引
...
1. 不能将索引用作表达式得一部分，也不能是函数的参数

   ```mysql
   select * from aaa where id+1=1
...
2. 索引不要进行类型转化
   ...
   ```mysql
   select * from sex from teah where tname=111;   #tname原来类型为字符串
   ```
...
2. 最佳左前缀
3. 删除多余索引
4. 调整查找条件，对索引有干扰的放在最后

##### 多表

1. 小表驱动大表
2. left join 给左边表添缩印，right join给右边表添索引
